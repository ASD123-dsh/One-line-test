...
class ByteEditor(QFrame):
    ...
    def init_ui(self, byte_value: int, description: str):
        ...
        grid.addWidget(title_label, 0, 0, alignment=Qt.AlignLeft | Qt.AlignVCenter)

        desc_label = QLabel(description or "")
        desc_label.setFont(QFont("Arial", 8))
        desc_label.setWordWrap(False)
        desc_label.setAlignment(Qt.AlignCenter)
        grid.addWidget(desc_label, 0, 1, alignment=Qt.AlignCenter)
        ...
        grid.addWidget(hex_widget, 1, 0, alignment=Qt.AlignLeft | Qt.AlignVCenter)
        ...
        self.dec_label.setFixedWidth(44)
        self.dec_label.setAlignment(Qt.AlignCenter)
        dec_layout.addWidget(self.dec_label)
        dec_layout.addStretch()
        grid.addWidget(dec_widget, 1, 1, alignment=Qt.AlignCenter)

        layout.addLayout(grid)

        bits_group = QGroupBox("位配置")
        bits_layout = QGridLayout()
        bits_layout.setContentsMargins(8, 6, 8, 6)
        bits_layout.setHorizontalSpacing(8)
        bits_layout.setVerticalSpacing(4)
        self.bit_checkboxes = []
        for i in range(8):
